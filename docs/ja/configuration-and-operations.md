# Laravel Security Guard: 詳細設定・運用リファレンス

[English quick start](../../README.md) | [日本語の導入手順](../../README.ja.md)

> この文書は設定リファレンス、モジュール別ガイド、運用上の注意をまとめた日本語資料です。初めて導入する場合は、先に[日本語の導入手順](../../README.ja.md)を完了してください。
...
2. proxy配下ではLaravelのtrusted proxyを設定し、stagingの実リクエストでclient IPが正しく解決されることを確認する
...
パッケージは`X-Forwarded-For`を独自に参照しません。proxy配下では**先にLaravelのtrusted proxy設定を完了**させ、stagingの実リクエストで`Request::ip()`が期待するclient IPを返すことを確認してから有効化してください。設定を誤ると全リクエストが同一IPと判定され、レート制限が誤作動します。`security-guard:status <ip>`は指定したIPの遮断・レート制限状態を表示するコマンドであり、実リクエストのIP解決結果を表示するものではありません。
